...
function ProductList() {
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      <div className="w-[80vw] h-auto py-10 px-5 flex-col justify-center ml-[50%] translate-x-[-50%] gap-4 ">
          ...
          return (
            <div key={index}>
              <div className="width-[100vw] height-[100vh] ">
                <h2
                  id="title"
                  className=" pt-10 text-black text-center text-4xl font-bold"
                >
                  {planttype.category}
                </h2>
              </div>

              <div className="flex w-[80vw] flex-wrap h-auto justify-center py-10 px-5 gap-10 col-span-3 ">
                {planttype.plants.map((plant, i) => {
                  return (
                    <div
                      id={i}
                      className="relative bg-white  w-1/4 py-4 px-3 flex flex-col items-center content-around shadow-md border-r-4"
                    >
                        <div className="absolute top-0 right-0 bg-red-400 py-1 px-3 text-sm text-white ">Sales</div>
                      <p className="text-black font-bold text-md pb-4 uppercase tracking-widest ">
                        {plant.name}
                      </p>
                      <img src={plant.image} className="w-4/5 h-[200px]"></img>
                      <p className="text-red-400 mt-2">{plant.cost}</p>
                      <p className="relative w-[80%] text-center py-4">
                        {plant.description}{" "}
                      </p>
                      <button className="bg-green-400 py-1 px-6 text-xl text-white mt-5 disabled:bg-gray-400" onClick={()=> dispatch(addToCart(plant))} 
